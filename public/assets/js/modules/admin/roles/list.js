$(function () {
    ...
    'columns': [
      {
        ...
        render: function (data, type, row) {
          return '<div class="btn-group">' +
            ...
            '    <a class="dropdown-item role-edit" id="' + row.id + '" href="javascript:;"><i class="far fa-edit icon-blue"></i>&nbsp;Editar</a>' +
            '    <a class="dropdown-item role-delete" id="' + row.id + '" href="javascript:;"><i class="far fa-trash-alt icon-red"></i>&nbsp;Excluir</a>' +
            '  </div>' +
            '</div>';
        }
      },
  ...
  $.validator.setDefaults({
    submitHandler: function () {
      ...
      var permissions = $('#permission_tree').jstree('get_selected');
      const data = {
      ...
      if (id) {
        ...
        insertRoute.execute(form.postType, urls.insert, data, form.dataTypeJson, form.contentJson, form.noCache, form.processData).then((data) => {
          dataTable.draw();
          $('#permission_tree').jstree(true).deselect_all();
          $('#inputId').val('')
        });
      }
    }
  ...
  $('body').on('click', 'a.role-edit', function () {
    ...
    editRoute.execute(urls.default + id, form.dataType).then((data) => {
      ...
      //$('#permission_tree').jstree(true).select_node([23,27]);
    }).catch()

  });

  $('body').on('click', 'a.role-delete', function () {
    const id = $(this).attr('id')
